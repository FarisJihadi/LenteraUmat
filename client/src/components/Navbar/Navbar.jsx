...
import setarainLogo from "../../assets/Navbar/setarain.png";
  ...
  return (
    ...
    >
      <nav className="px-6 940:px-16 py-6 flex justify-between items-center font-medium relative">
        ...
        <div className="w-fit">
          <Link to="/">
            {navVariant === 1 && <img src={setarainLogo} className="w-32 940:w-36" alt="Logo" />}
            {navVariant === 2 && <img src={setarainLogo2} className="w-32 940:w-36  " alt="Logo" />}
          </Link>
        </div>
        ...
        >
          <div className="w-full flex-col flex 940:flex-row py-16 940:py-0 text-center justify-center gap-10">
            <Link to={paths.home} className={menuOpen ? "text-primary" : "text-white" + navLinkClass(isLinkActive(paths.home))}>
              Beranda
            </Link>
            {user?.role === "komunitas" ? (
              ""
            ) : (
              <Link to={paths.upload} className={menuOpen ? "text-primary" : "text-white" + navLinkClass(isLinkActive(paths.upload))}>
                Upload Donasi
              </Link>
            )}
            <Link to={paths.donations} className={menuOpen ? "text-primary" : "text-white" + navLinkClass(isLinkActive(paths.donations))}>
              Lihat Donasi
            </Link>
            <Link to={paths.artikel} className={menuOpen ? "text-primary" : "text-white" + navLinkClass(isLinkActive(paths.artikel))}>
              Artikel
            </Link>
            {!user && (
