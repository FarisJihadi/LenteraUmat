...
import heroImage2 from "../../assets/LihatDonasi/hero2.png";
...
export default function CariDonasiForm({ setDonations, loading, setLoading, setIsSearched }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center pt-64 pb-24 p-4 h-auto md:py-8 sm:px-12 md:h-screen mb-12 relative">
      <HeroSection />
      <form onSubmit={handleSubmit} className="flex w-full max-w-4xl overflow-visible relative">
        {/* Dropdown provinsi */}
        <div className="relative">
          <div className="bg-white flex justify-between items-center p-6 rounded-l-xl md:gap-2 gap-1 max-w-60 border-r-[1px] border-[#D9D9D9] cursor-pointer" onClick={() => setIsOpenProvinsi(!isOpenProvinsi)}>
            <img src={locationIcon} alt="iconLocation" className="w-4 mr-2" />
            <span className="text-smallText font-semibold md:block hidden">{selectedProvinsi || "Provinsi"}</span>
            <img src={arrowDown} alt="arrowDown" className={`md:w-3 w-2  mx-2 md:block hidden ${isOpenProvinsi ? "transform rotate-180 md:w-3 w-2" : ""}`} />
          </div>
              ...
              <ul className="p-2">
                ...
                {Indonesia.map((provinsi) => (
                  <li
                    ...
                    onClick={() => {
                      handleProvinsiChange({ target: { value: provinsi.namaProvinsi } });
                      setIsOpenProvinsi(false);
                    }}
                  >
        ...
        {/* Input keyword */}
        <input type="text" className="flex-1 p-5 shadow-[0px_0px_6px_2px_rgba(0,0,0,0.1)] rounded-r-xl border-gray-300 focus:outline-none" placeholder="Cari Donasi" value={searchKeyword} onChange={handleKeywordChange} />

        {loading && <img src={loadingGif} className="w-12 h-12 absolute right-24 top-3" alt="loadingGif" />}

        {/* Tombol Submit */}
        <button type="submit" className="bg-primary py-2 px-4 rounded-md absolute right-5 top-[20%]" aria-label="Search button">
...
function HeroSection() {
  return (
    <>
      <img className="absolute z-[-1] top-0 left-0 w-full h-full object-cover object-center" src={heroImage2} alt="Hero background" />
      <div className="absolute inset-0 bg-black bg-opacity-30"></div>
      <div className="relative z-10 text-center px-4 sm:px-0">
        <h1 className="text-4xl sm:text-heading-xl text-white font-extrabold mb-4 sm:mb-6 leading-snug">Donasi Pendidikan</h1>
        <h2 className="text-sm sm:text-body-xl text-white font-medium mb-6 sm:mb-12 leading-relaxed">Bersama Kita Wujudkan Pendidikan yang Setara</h2>
      </div>
    </>
  );
}
